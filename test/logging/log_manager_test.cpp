...
TEST(LogManagerTest, BasicLogging) {
  ...
  LOG_DEBUG("finish InsertTuple()");
  EXPECT_TRUE(test_table->MarkDelete(rid, txn));
  LOG_DEBUG("finish MarkDelete()");
  ...
  // verify size of log file is 0, where disk_manager_->ReadLog() returns false;
  storage_engine = new StorageEngine("test.db");
  EXPECT_FALSE(storage_engine->disk_manager_->ReadLog(buffer, PAGE_SIZE, 0));
  delete storage_engine;
  LOG_DEBUG("PATH:%s", getcwd(buffer, PAGE_SIZE));
  remove("test.db");
  remove("test.log");
}
